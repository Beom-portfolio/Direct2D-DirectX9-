...
void CMapTool::OnBnClickedButtonSave()
{
	...
	Dlg.m_ofn.lpstrInitialDir = L"..\\Data\\Map";

	vector<TILE*>* pVecTile = ((CMainFrame*)AfxGetMainWnd())->GetMainView()->GetMainTile()->GetTile();
	...
	m_BackInfo.iBackType = ((CMainFrame*)AfxGetMainWnd())->GetMainView()->GetMainBack()
		->GetBackIntType();

	WriteFile(hFile, &m_BackInfo, sizeof(BACKINFO), &dwByte, NULL);
	
	m_BackCount = ((CMainFrame*)AfxGetMainWnd())->GetMainView()->GetMainTile()
		->GetXYCount();

	WriteFile(hFile, &m_BackCount, sizeof(XYCOUNT), &dwByte, NULL);
...
void CMapTool::OnBnClickedButtonLoad()
{
	...
	pMain->GetMainBack()->SetStageTypeFromLoad(m_BackInfo.iBackType);

	pMain->GetMainTile()->SetXYCount(m_BackCount);
