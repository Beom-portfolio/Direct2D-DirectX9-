...
void CPathFind::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_PAHT, m_listboxPathFind);
}


BEGIN_MESSAGE_MAP(CPathFind, CDialog)
	ON_LBN_SELCHANGE(IDC_LIST_PAHT, &CPathFind::OnLbnSelchangeListPath)
...
void CPathFind::OnBnClickedButtonLoad()
{
	...
	while( !LoadFile.eof() )
	{
		IMAGE_PATH* Temp = new IMAGE_PATH;
		...
		Temp->wstrObjKey = szObjKey;
		Temp->wstrStateKey = szStateKey;
		Temp->iCount = _wtoi(szCount);
		Temp->wstrPath = szPath;

		m_listboxPathFind.AddString(wstrCombine.c_str());
		m_listImagePath.push_back(Temp);
	}
