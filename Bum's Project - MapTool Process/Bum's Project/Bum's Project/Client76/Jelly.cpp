...
void CJelly::Render(void)
{
	if (m_TargetCheck == true)
	{
		...
		float fY = CENTER_POS(pTargetTex->ImageInfo.Height) - 10;
...
void CJelly::MonsterState(void)
{
	...
	if (m_bChangeCheck == true)
	{
		...
		case MONSTER_STAND:
			...
			break;
		case MONSTER_WALK:
			m_wstrObjKey = L"J_WALK";
			m_tFrame = FRAME(0.f, 6.f, 6.f);
			break;
		case MONSTER_ATTACK:
			m_wstrObjKey = L"J_ATTACK";
			m_tFrame = FRAME(0.f, 17.f, 17.f);
			m_MonsterAttackCheck = true;
			break;
		case MONSTER_DAMAGE:
			m_wstrObjKey = L"J_DAMAGE";
...
void CJelly::MonsterDir(int MoveIndex)
{
...
void CJelly::MyMatrixCheck(void)
{
	D3DXMATRIX matTrans;
	D3DXMATRIX matScale;
...
void CJelly::MyFrameCheck(void)
{
	if (m_bAttachCheck == true)
	{
		...
		if (m_tFrame.fIndex >= 10.f && m_MonsterAttackCheck == true)
		{
...
void CJelly::MonsterDamage(void)
{
